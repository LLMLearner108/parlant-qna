...
from parlant_qna.default_prompt_sections import *
...
class App:
    ...
    async def _build_prompt(
        ...
        prompt_builder = PromptBuilder(
            on_build=lambda prompt: self.logger.debug(f"Prompt:\n{prompt}")
        )

        # Add the identity of the agent
        prompt_builder.add_section(
            name="agent-identity", template=agent_identity, props={}
        )

        # Add general task-based instructions
        prompt_builder.add_section(
            name="general-task-based-instructions",
            template=general_task_based_instructions,
            props={},
        )

        # Add specific task-based instructions
        prompt_builder.add_section(
            name="specific-task-based-instructions",
            template=specific_task_based_instructions,
            props={},
        )
        ...
        # Add the query-specific questions and answers which are relevant to the user's query
        prompt_builder.add_section(
            name="query-specific-questions-and-answers",
            template="""
Background Information: ###
{background_info}
###
        """,
            props={"background_info": background_info},
        )
...
###
    ...
    async def ask_question(
        ...
        prompt = await self._build_prompt(question, tags)
